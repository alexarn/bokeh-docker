...
(defvar phpunit-function-setup-regexp
  "^\\s-*\\(\\(?:\\(?:abstract\\|final\\|private\\|protected\\|public\\|static\\)\\s-+\\)*\\)function\\s-+\\(\\(?:setup\\|setUp\\)+\\)\\s-*("
  "Regexp that match function setup declaration")
...
(defun phpunit-function-setup-ap ()
  "Get current class name for setup"
  (save-excursion
    (goto-char (line-end-position))
    (re-search-backward phpunit-function-setup-regexp nil t))
)
